 ...
 *
 *   use \App\Models\Concerns\Nestable;
 *
 * In the migration class definition:
 *
...
trait Nestable
{
    ...
    {
        return ($this->getLeft() > $node->getLeft() && $this->getLeft() < $node->getRight());
    }
    ...
    public function scopeLeaves($query)
    {
        ...
        return $query->allChildren()->whereRaw($rightCol . ' - ' . $leftCol . ' = 1');
    }
    ...
    {
        return intval(($this->getRight() - $this->getLeft() - 1) / 2);
    }
    ...
    {
        $highRight = $this->newQueryWithoutScopes()
    ...
    protected function performMove($node, $target, $position)
    {
        ...
        $parentId = ($position == 'child') ? $target->getKey() : $target->getParentId();
        if ($parentId === null) {
            $parentId = 'NULL';
        } else {
